...
def handle_message(update: Update, context: CallbackContext):
    ...
    try:
        ...
        # Simple audio download (no conversion - Telegram plays m4a directly)
        ydl_opts = {
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if filepath.endswith('.webm'):
                new_filepath = filepath.replace('.webm', '.m4a')
        ...
        if "Sign in to confirm you're not a bot" in error_msg:
            update.message.reply_text("❌ This video requires human verification (CAPTCHA). Try a different public video.")
